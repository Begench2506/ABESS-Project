...
QByteArray send_answ(QString task, QString cor, QString socket){
    ...
    bool result = false;
    qDebug() << task << " <$ " << cor << '\n';
    std::string corStr = cor.toStdString();
        bool hasEulerCycle = checkEulerCycle(corStr);
    if(task == '1') //Sunnat
    {
       if (hasEulerCycle == true)
        {
            qDebug() << "Good answer";
            result = true;
        }
        else if (hasEulerCycle == false)
       {
            qDebug() << "Wrong answer";
        }
        else
    ...
    //else if(task == '5')
    //{}
    MyDataBase::getInstance()->correct(task, result, socket);
    return res;
}
